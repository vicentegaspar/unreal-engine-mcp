...
def create_rollercoaster(
    track_length: int = 20,  # Number of track segments
    max_height: float = 2000.0,  # Maximum height of the coaster
    location: List[float] = [0.0, 0.0, 0.0],
    name_prefix: str = "Coaster",
    include_physics_carts: bool = True,
    loop_count: int = 2  # Number of loops in the track
) -> Dict[str, Any]:
    """
    Create an epic rollercoaster with tracks, supports, loops, and physics-enabled carts.
    This creates a visually stunning coaster perfect for TikTok videos with dramatic
    camera angles showing the full track layout and bouncing physics carts.
    """
    try:
        unreal = get_unreal_connection()
        if not unreal:
        ...
        logger.info(f"Creating rollercoaster with {track_length} segments and {loop_count} loops")
        all_actors = []
        
        # Track parameters
        segment_length = 400.0
        track_width = 200.0
        support_spacing = 800.0
        
        # Generate track path with hills, drops, and loops
        track_points = []
        current_x = location[0]
        current_y = location[1]
        current_z = location[2] + 500  # Start elevated
        
        for i in range(track_length):
            # Create varied track path
            progress = i / track_length
            
            # Add loops at specific points
            if loop_count > 0 and i % (track_length // (loop_count + 1)) == track_length // (loop_count * 2):
                # Create a loop
                loop_radius = 400.0
                loop_segments = 8
                for j in range(loop_segments):
                    loop_angle = (2 * math.pi * j) / loop_segments
                    loop_x = current_x + segment_length * (j / loop_segments)
                    loop_y = current_y
                    loop_z = current_z + loop_radius + loop_radius * math.cos(loop_angle)
                    track_points.append([loop_x, loop_y, loop_z])
                current_x += segment_length
            else:
                # Regular track with hills and drops
                height_variation = math.sin(progress * math.pi * 4) * (max_height / 2)
                # Big drop at 25% and 75%
                if 0.23 < progress < 0.27:
                    height_variation = -max_height * 0.7
                elif 0.73 < progress < 0.77:
                    height_variation = -max_height * 0.5
                
                current_x += segment_length
                # Add curves to the track
                current_y += math.sin(progress * math.pi * 6) * 300
                current_z = location[2] + 500 + height_variation
                
                track_points.append([current_x, current_y, current_z])
        
        # Build track segments
        logger.info("Building track segments...")
        for i in range(len(track_points) - 1):
            start_point = track_points[i]
            end_point = track_points[i + 1]
            
            # Calculate track segment position and rotation
            segment_pos = [
            ...
            # Calculate rotation to align track
            ...
            # Main track rail (left)
            left_rail_name = f"{name_prefix}_TrackLeft_{i}"
            left_offset_y = track_width / 2
            left_rail_result = unreal.send_command("spawn_actor", {
                "name": left_rail_name,
                "type": "StaticMeshActor",
                "location": [
                    segment_pos[0] - math.sin(yaw * math.pi/180) * left_offset_y,
                    segment_pos[1] + math.cos(yaw * math.pi/180) * left_offset_y,
                    segment_pos[2]
                ],
                "rotation": [pitch, yaw, 0],
                "scale": [segment_length/100, 0.2, 0.1],
                "static_mesh": "/Engine/BasicShapes/Cube.Cube"
            })
            if left_rail_result and left_rail_result.get("status") == "success":
                all_actors.append(left_rail_result.get("result"))
            
            # Main track rail (right)
            right_rail_name = f"{name_prefix}_TrackRight_{i}"
            right_offset_y = -track_width / 2
            right_rail_result = unreal.send_command("spawn_actor", {
                "name": right_rail_name,
                "type": "StaticMeshActor",
                "location": [
                    segment_pos[0] - math.sin(yaw * math.pi/180) * right_offset_y,
                    segment_pos[1] + math.cos(yaw * math.pi/180) * right_offset_y,
                    segment_pos[2]
                ],
                "rotation": [pitch, yaw, 0],
                "scale": [segment_length/100, 0.2, 0.1],
            ...
            if right_rail_result and right_rail_result.get("status") == "success":
                all_actors.append(right_rail_result.get("result"))
            
            # Cross ties between rails
            if i % 2 == 0:  # Every other segment for performance
                for j in range(3):  # 3 ties per segment
                    tie_offset = (j - 1) * segment_length / 3
                    tie_x = start_point[0] + (dx * (j + 1) / 4)
                    tie_y = start_point[1] + (dy * (j + 1) / 4)
                    tie_z = start_point[2] + (dz * (j + 1) / 4)
                    
                    tie_name = f"{name_prefix}_Tie_{i}_{j}"
                    tie_result = unreal.send_command("spawn_actor", {
                        "name": tie_name,
                        "type": "StaticMeshActor",
                        "location": [tie_x, tie_y, tie_z],
                        "rotation": [0, yaw, 0],
                        "scale": [0.1, track_width/100 * 1.2, 0.05],
                        "static_mesh": "/Engine/BasicShapes/Cube.Cube"
                    })
                    if tie_result and tie_result.get("status") == "success":
                        all_actors.append(tie_result.get("result"))
        
        # Add support pillars
        logger.info("Adding support structures...")
        support_count = int(len(track_points) / 4)
        for i in range(support_count):
            point_index = int(i * len(track_points) / support_count)
            if point_index < len(track_points):
                support_point = track_points[point_index]
                ground_height = location[2]
                pillar_height = support_point[2] - ground_height
                
                if pillar_height > 100:  # Only add supports for elevated sections
                    # Main support pillar
                    pillar_name = f"{name_prefix}_Support_{i}"
                    pillar_result = unreal.send_command("spawn_actor", {
                        "name": pillar_name,
                        "type": "StaticMeshActor",
                        "location": [
                            support_point[0],
                            support_point[1],
                            ground_height + pillar_height / 2
                        ],
                        "scale": [0.5, 0.5, pillar_height / 100],
                        "static_mesh": "/Engine/BasicShapes/Cylinder.Cylinder"
                    })
                    if pillar_result and pillar_result.get("status") == "success":
                    ...
                    if pillar_height > 500:
                        for brace_level in range(int(pillar_height / 300)):
                            brace_height = ground_height + (brace_level + 1) * 300
                            # X-brace left
                            brace_name_l = f"{name_prefix}_BraceL_{i}_{brace_level}"
                            brace_result_l = unreal.send_command("spawn_actor", {
                                "name": brace_name_l,
                                "type": "StaticMeshActor",
                                "location": [
                                    support_point[0] - 50,
                            ...
                            if brace_result_l and brace_result_l.get("status") == "success":
                                all_actors.append(brace_result_l.get("result"))
                            
                            # X-brace right
                            brace_name_r = f"{name_prefix}_BraceR_{i}_{brace_level}"
                            brace_result_r = unreal.send_command("spawn_actor", {
                                "name": brace_name_r,
                                "type": "StaticMeshActor",
                                "location": [
                                    support_point[0] + 50,
                            ...
                            if brace_result_r and brace_result_r.get("status") == "success":
                                all_actors.append(brace_result_r.get("result"))
        
        # Create station platform at start
        logger.info("Building station platform...")
        station_result = unreal.send_command("spawn_actor", {
            "name": f"{name_prefix}_Station",
            "type": "StaticMeshActor",
            "location": [location[0], location[1], location[2] + 450],
            "scale": [6.0, 4.0, 0.5],
            "static_mesh": "/Engine/BasicShapes/Cube.Cube"
        })
        if station_result and station_result.get("status") == "success":
            all_actors.append(station_result.get("result"))
        
        # Station roof
        roof_result = unreal.send_command("spawn_actor", {
            "name": f"{name_prefix}_StationRoof",
            "type": "StaticMeshActor",
            "location": [location[0], location[1], location[2] + 750],
            "scale": [7.0, 5.0, 0.2],
            "static_mesh": "/Engine/BasicShapes/Cube.Cube"
        })
        if roof_result and roof_result.get("status") == "success":
        ...
        # Add physics-enabled carts if requested
        if include_physics_carts:
            logger.info("Spawning physics-enabled roller coaster carts...")
            cart_colors = [
                [1.0, 0.0, 0.0, 1.0],  # Red
            ...
            # Spawn multiple carts at different points on the track
            num_carts = min(5, len(track_points) // 4)
            for cart_index in range(num_carts):
                # Place carts at different heights for dramatic effect
                spawn_point_index = cart_index * 3
                if spawn_point_index < len(track_points):
                    spawn_point = track_points[spawn_point_index]
                    cart_name = f"{name_prefix}_Cart_{cart_index}"
                    
                    # Create physics cart blueprint
                    bp_name = f"{cart_name}_BP"
                    bp_result = unreal.send_command("create_blueprint", {
                        "name": bp_name,
                        "parent_class": "Actor"
                    })
                    
                    if bp_result and bp_result.get("status") == "success":
                        # Add cart body
                        body_result = unreal.send_command("add_component_to_blueprint", {
                            "blueprint_name": bp_name,
                            "component_type": "StaticMeshComponent",
                            "component_name": "CartBody",
                            "scale": [1.5, 1.0, 0.8],
                        ...
                        # Enable physics with specific settings for rolling
                        ...
                        color_result = unreal.send_command("set_mesh_material_color", {
                            "blueprint_name": bp_name,
                            "component_name": "CartBody",
                            "color": cart_colors[cart_index % len(cart_colors)],
                            "material_path": "/Engine/BasicShapes/BasicShapeMaterial"
                        })
                        
                        # Add wheels (visual only)
                        for wheel_index in range(4):
                            wheel_x = 1.2 if wheel_index < 2 else -1.2
                            wheel_y = 0.8 if wheel_index % 2 == 0 else -0.8
                            wheel_name = f"Wheel_{wheel_index}"
                            
                            wheel_result = unreal.send_command("add_component_to_blueprint", {
                                "blueprint_name": bp_name,
                                "component_type": "StaticMeshComponent",
                                "component_name": wheel_name,
                                "location": [wheel_x * 50, wheel_y * 50, -40],
                                "scale": [0.3, 0.3, 0.1],
                                "component_properties": {}
                            })
                            
                            wheel_mesh_result = unreal.send_command("set_static_mesh_properties", {
                                "blueprint_name": bp_name,
                                "component_name": wheel_name,
                                "static_mesh": "/Engine/BasicShapes/Cylinder.Cylinder"
                            })
                        
                        # Compile the cart blueprint
                        compile_result = unreal.send_command("compile_blueprint", {
                            "blueprint_name": bp_name
                        })
                        
                        # Spawn the cart on the track
                        cart_spawn_result = unreal.send_command("spawn_blueprint_actor", {
                            "blueprint_name": bp_name,
                            "actor_name": cart_name,
                            "location": [
                                spawn_point[0],
                                spawn_point[1],
                                spawn_point[2] + 100  # Spawn above track to drop
                            ]
                        })
                        
                        if cart_spawn_result and cart_spawn_result.get("status") == "success":
                            all_actors.append(cart_spawn_result.get("result"))
        ...
        # Entry sign
        sign_result = unreal.send_command("spawn_actor", {
            "name": f"{name_prefix}_EntranceSign",
            "type": "StaticMeshActor",
            "location": [location[0] - 300, location[1], location[2] + 700],
            "scale": [4.0, 0.2, 2.0],
            "static_mesh": "/Engine/BasicShapes/Cube.Cube"
        })
        if sign_result and sign_result.get("status") == "success":
            all_actors.append(sign_result.get("result"))
        
        # Flags along the track
        flag_spacing = len(track_points) // 10
        for i in range(0, len(track_points), flag_spacing):
            if i < len(track_points):
                flag_point = track_points[i]
                flag_name = f"{name_prefix}_Flag_{i}"
                flag_result = unreal.send_command("spawn_actor", {
                    "name": flag_name,
                    "type": "StaticMeshActor",
                    "location": [flag_point[0], flag_point[1] - 300, flag_point[2] + 200],
                    "scale": [0.1, 2.0, 1.5],
                    "static_mesh": "/Engine/BasicShapes/Cube.Cube"
                })
        ...
        logger.info(f"Rollercoaster creation complete! Created {len(all_actors)} actors")
        
        return {
            "success": True,
            "message": f"Epic rollercoaster created with {track_length} track segments, {loop_count} loops, and {len(all_actors)} total elements!",
            "actors": all_actors,
            "stats": {
                "track_segments": track_length,
                "loops": loop_count,
                "max_height": max_height,
                "total_actors": len(all_actors),
                "has_physics_carts": include_physics_carts
            }
        }
        
    except Exception as e:
        logger.error(f"create_rollercoaster error: {e}")
        return {"success": False, "message": str(e)}
