...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    else if (CommandType == TEXT("set_physics_material_properties"))
    {
        return HandleSetPhysicsMaterialProperties(Params);
    }
    else if (CommandType == TEXT("set_collision_properties"))
    {
        return HandleSetCollisionProperties(Params);
    }
    else if (CommandType == TEXT("set_body_instance_properties"))
    {
        return HandleSetBodyInstanceProperties(Params);
    }
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleSetComponentProperty(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!Blueprint->SimpleConstructionScript)
    {
        ...
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Invalid blueprint construction script"));
    }
    ...
    if (!ComponentTemplate)
    {
        ...
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Invalid component template"));
    }
            ...
            if (FStructProperty* StructProp = CastField<FStructProperty>(Property))
            {
                ...
                else
                {
                    ...
                    UE_LOG(LogTemp, Log, TEXT("SetComponentProperty - Using generic struct handler for %s"), 
                        *PropertyName);
                    bSuccess = FEpicUnrealMCPCommonUtils::SetObjectProperty(ComponentTemplate, PropertyName, JsonValue, ErrorMessage);
                    if (!bSuccess)
                    {
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleSetPhysicsMaterialProperties(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    {
    ...
    if (Params->HasField(TEXT("restitution")))
    {
        Restitution = Params->GetNumberField(TEXT("restitution"));
    }
    double Friction = 0.3;
    if (Params->HasField(TEXT("friction")))
    {
        Friction = Params->GetNumberField(TEXT("friction"));
    }
    double Density = 1.0;
    ...
    PrimComponent->SetPhysMaterialOverride(PhysMat);
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleSetCollisionProperties(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    {
    ...
    FString CollisionEnabledStr = TEXT("QueryAndPhysics");
    Params->TryGetStringField(TEXT("collision_enabled"), CollisionEnabledStr);
    ...
    FString CollisionResponseStr = TEXT("Block");
    Params->TryGetStringField(TEXT("collision_response"), CollisionResponseStr);
    ECollisionResponse CollisionResponse = ECollisionResponse::ECR_Block;
    if (CollisionResponseStr == TEXT("Ignore"))
    {
        CollisionResponse = ECollisionResponse::ECR_Ignore;
    }
    else if (CollisionResponseStr == TEXT("Overlap"))
    {
        CollisionResponse = ECollisionResponse::ECR_Overlap;
    }

    bool bGenerateHit = true;
    if (Params->HasField(TEXT("generate_hit_events")))
    {
        bGenerateHit = Params->GetBoolField(TEXT("generate_hit_events"));
    }

    PrimComponent->SetCollisionEnabled(CollisionEnabled);
    PrimComponent->SetCollisionResponseToAllChannels(CollisionResponse);
    PrimComponent->SetNotifyRigidBodyCollision(bGenerateHit);
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleSetBodyInstanceProperties(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    {
    ...
    bool bLockX = false;
    Params->TryGetBoolField(TEXT("lock_x_rotation"), bLockX);
    bool bLockY = false;
    Params->TryGetBoolField(TEXT("lock_y_rotation"), bLockY);
    bool bLockZ = false;
    Params->TryGetBoolField(TEXT("lock_z_rotation"), bLockZ);
    bool bOverrideMass = true;
    Params->TryGetBoolField(TEXT("override_mass"), bOverrideMass);

    PrimComponent->BodyInstance.bLockXRotation = bLockX;
    PrimComponent->BodyInstance.bLockYRotation = bLockY;
    PrimComponent->BodyInstance.bLockZRotation = bLockZ;
    PrimComponent->BodyInstance.bOverrideMass = bOverrideMass;
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleSetBlueprintProperty(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!DefaultObject)
    {
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Failed to get default object"));
    }
    ...
    if (Params->HasField(TEXT("property_value")))
    {
        ...
        FString ErrorMessage;
        if (FEpicUnrealMCPCommonUtils::SetObjectProperty(DefaultObject, PropertyName, JsonValue, ErrorMessage))
        {
            // Mark the blueprint as modified
            FBlueprintEditorUtils::MarkBlueprintAsModified(Blueprint);
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleSetPawnProperties(const TSharedPtr<FJsonObject>& Params)
{
    // Get required parameters
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    ...
    if (Params->HasField(TEXT("auto_possess_player")))
    {
        ...
        FString ErrorMessage;
        if (FEpicUnrealMCPCommonUtils::SetObjectProperty(DefaultObject, TEXT("AutoPossessPlayer"), AutoPossessValue, ErrorMessage))
        {
    ...
    for (int32 i = 0; i < 3; i++)
    {
        if (Params->HasField(ParamNames[i]))
        {
            ...
            FString ErrorMessage;
            if (FEpicUnrealMCPCommonUtils::SetObjectProperty(DefaultObject, RotationProps[i], Value, ErrorMessage))
            {
                bAnyPropertiesSet = true;
                TSharedPtr<FJsonObject> PropResultObj = MakeShared<FJsonObject>();
                PropResultObj->SetBoolField(TEXT("success"), true);
    ...
    if (Params->HasField(TEXT("can_be_damaged")))
    {
        ...
        FString ErrorMessage;
        if (FEpicUnrealMCPCommonUtils::SetObjectProperty(DefaultObject, TEXT("bCanBeDamaged"), Value, ErrorMessage))
        {
            bAnyPropertiesSet = true;
            TSharedPtr<FJsonObject> PropResultObj = MakeShared<FJsonObject>();
            PropResultObj->SetBoolField(TEXT("success"), true);
    ...
    else if (ResultsObj->Values.Num() == 0)
    {
        ...
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(TEXT("No properties specified to set"));
    }
